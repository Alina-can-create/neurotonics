    ...
    return (
      <main className="bg-white min-h-screen flex items-center justify-center">
        <div className="text-center px-4">
          ...
          <Link
            href="/product"
            className="inline-flex items-center px-6 py-3 bg-green-700 hover:bg-green-800 text-white font-semibold rounded-xl transition-all duration-300"
          >
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map((item) => (
              <div key={item.id} className="flex items-center gap-4 p-4 sm:p-6 bg-white rounded-2xl border border-gray-200">
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-green-50 flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-green-600 font-semibold">${item.price.toFixed(2)} AUD</p>
                </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl border border-gray-200 p-6 sticky top-24">
                ...
                <div className="pt-3 border-t border-gray-200">
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      maxLength={4}
                      className="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 placeholder-gray-400 text-sm focus:outline-none focus:border-green-500"
                    />
                    <button
                      onClick={handleShippingCalculation}
                      disabled={shippingLoading || postcode.length !== 4}
                      className="px-3 py-2 bg-green-50 text-green-700 border border-green-200 rounded-lg hover:bg-green-100 transition-colors disabled:opacity-50 text-xs font-medium"
                    >
                      {shippingLoading ? '...' : 'Go'}
                    </button>
                  </div>
              ...
              <Link
                ...
                className="block w-full mt-6 py-3.5 bg-green-700 hover:bg-green-800 text-white font-semibold rounded-xl transition-all duration-300 text-center"
              >
                Proceed to Checkout
              </Link>
              ...
              <Link
                href="/product"
                className="block w-full mt-3 py-2.5 text-center text-green-700 hover:text-green-600 text-sm transition-colors"
              >
                ← Continue Shopping
              </Link>
            </div>
